...
export const PerformanceAccordion = ({ tags }: IPerformanceAccordion) => {
  ...
  return (
    <S.List>
      {tags.map((tag) => {
        const isOpen = openIds.includes(tag.id_tag);
        const average = getAverage(tag.subtags);

        return (
          <S.Item key={tag.id_tag}>
            <S.Header
              type="button"
