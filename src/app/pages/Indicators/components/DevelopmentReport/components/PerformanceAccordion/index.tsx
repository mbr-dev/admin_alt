...
export const PerformanceAccordion = ({ tags, forceExpanded = false, isExporting = false }: IPerformanceAccordion) => {
  const { t } = useTranslation("indicators");
  const [openIds, setOpenIds] = useState<number[]>(tags.length ? [tags[0].id_tag] : []);
  const allIds = tags.map((tag) => tag.id_tag);
  ...
  return (
    <S.Card data-export-performance={isExporting ? "" : undefined}>
      <S.Title>{t("dev_competencyTitle")}</S.Title>
      <S.Subtitle>{t("dev_competencySubtitle")}</S.Subtitle>
          ...
          return (
            <S.Item key={tag.id_tag} $exporting={isExporting}>
              <S.Header
                ...
                aria-controls={`category-${tag.id_tag}`}
              ...
              {isOpen && (
                <S.Body id={`category-${tag.id_tag}`}>
                  {tag.subtags.map((subtag) => (
                    <S.Row key={subtag.id_subtag} $exporting={isExporting}>
                      <S.Skill $exporting={isExporting}>{subtag.subtag}</S.Skill>
                      <S.BarBox>
