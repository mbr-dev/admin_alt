...
export const ModalIAP = ({ show, showDoubt }: IModal) => {
  const { t } = useTranslation("indicators");

  return (
    <Dialog.Dialog open={show} onOpenChange={() => showDoubt(0, false)}>
      <Dialog.DialogContent className="max-w-[300px] justify-center landscape:max-w-[550px] md:max-w-[600px] md:rounded-xl landscape:lg:rounded-xl landscape:lg:max-w-[650px] rounded-xl border-2 border-mbr-blue-10 overflow-hidden">
        <Dialog.DialogHeader>
          <Dialog.DialogTitle className="text-mbr-blue-10 font-bold text-center text-xl md:text-2xl landscape:lg:text-2xl">{t("iap_title")}</Dialog.DialogTitle>
        </Dialog.DialogHeader>

        <S.Main>
          <S.Text>{t("iap_textTop")}</S.Text>
          <S.Div>
          ...
          <S.Text>{t("iap_textBottom")}</S.Text>
        </S.Main>
